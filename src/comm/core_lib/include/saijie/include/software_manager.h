...
#ifndef S330_BASICS_SOFTWARE_MANAGER_H
#define S330_BASICS_SOFTWARE_MANAGER_H
#include <mutex>
...
constexpr char kPubSoftwareModuleTopic[]    = "/alg_to_core_software";///< 算法模块 ---->core模块
constexpr char kSubSoftwareModuleTopic[]    = "/core_to_alg_software";///< core模块 ---->算法模块
...
public:
    /**
     * 创建一个SoftwareManager对象，如果存在则不重新创建，返回之前创建的对象
     * @return SoftwareManager对象指针
     */
    static SoftwareManager* Create();
    /**
...
#pragma GCC diagnostic pop
        return ReqList(MsgDomain::EM_MSG_DOMAIN_SOFTWARE_MANAGER, msg_type, fun, fp1, obj1);
    }
    /**
     * 取消注册回调
     * @tparam T
...
#pragma GCC diagnostic pop
     ...
     * 例如：{"MsgType":4,"TransId":0,"MsgContent":{...}}
     * @return 0：成功，其他失败
     */
...
private:
     ...
     */
    ~SoftwareManager();
    SoftwareManager(const SoftwareManager& signal) = delete;
    const SoftwareManager& operator=(const SoftwareManager& signal) = delete;
    /**
     * 线程死循环
     * @param arg void*类型指针
     */
...
private:
    static SoftwareManager* m_instance_;///< 对象指针
...
#endif //S330_BASICS_SOFTWARE_MANAGER_H
