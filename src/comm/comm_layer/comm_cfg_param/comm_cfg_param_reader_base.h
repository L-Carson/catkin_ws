#ifndef __COMM_CFG_PARAM_READER_BASE_H__
#define __COMM_CFG_PARAM_READER_BASE_H__
...
修改记录:
**************************************************************************************/
class CFG_PARAM_READER_BASE_C
{
public:
protected:
    template<typename T>
    int GetAttribute(TiXmlElement *pobjCfgParam, const char *attribute, T &value)
    {
        if (!pobjCfgParam) {
    ...
    int Char2Num(char c);
    int StringHex2Num(string str);

    int GetAttribute(TiXmlElement *pobjCfgParam, const char *attribute, bool &value);
