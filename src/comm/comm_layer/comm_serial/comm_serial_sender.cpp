...
int SERIAL_SENDER_C::Send(SERIAL_ADDR_ENUM enTgtAddr, unsigned char msgType, const unsigned char *pucMsg, unsigned short msgLen)
{
    std::unique_lock<std::mutex> objLocker(objMutex_);
    ...
    int rsltOfEncode = objFrame.EncodeReq(enTgtAddr, AllocFrameNo(enTgtAddr), msgType, pucMsg, msgLen, stFrameBuff);
...
int SERIAL_SENDER_C::SendPass(const unsigned char *pucData, unsigned short dataLen)
{
    std::unique_lock<std::mutex> objLocker(objMutex_);
    ...
    SERIAL_FRAME_C objFrame(enLocAddr_);
    int rsltOfDecode = objFrame.Decode(pucData, dataLen);
...
unsigned char SERIAL_SENDER_C::AllocFrameNo(SERIAL_ADDR_ENUM enTgtAddr)
{
    R_ASSERT_1(enTgtAddr < ARRAY_SIZE(aucAddrFrameNo_), std::numeric_limits<unsigned char>::max(), enTgtAddr);
...
修改记录:
**************************************************************************************/
int SERIAL_SENDER_C::SendFrameSequentially(const SERIAL_FRAME_C &objFrame)
{
    ++stDbgInfo_.msgCount;
    ...
    int rsltOfBuff = objFrame.GetBuff(stBuff);
...
void SERIAL_SENDER_C::SendHandshakeMsg(void)
{
    ...
    int rsltOfSend = SendFrameSequentially(objFrame);
...
{
    SERIAL_FRAME_BUFF_P_STRU stBuff;
    int rsltOfGetFrameBuff = objSendingFrame_.GetBuff(stBuff);
