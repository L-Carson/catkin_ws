...
struct SERIAL_SENDER_DBG_INFO_STRU {
    unsigned int msgCount;                            /* 总的发送消息数，包含应用层消息和握手消息 */
    unsigned int msgCountOfApp;                       /* 应用方发送的消息数 */
    unsigned int msgConutOfAppFailAsDisconnect;       /* 因断链导致应用发送失败的消息数 */
    unsigned int msgCountOfHandshake;                 /* 发送的握手消息数 */
    unsigned int msgCountOfFailAsBusy;                /* 因链路忙导致丢弃而失败的消息数 */
    unsigned int msgCountOfFailOnSending;             /* 多次尝试最终仍失败的消息数 */

    unsigned int msgLenOfAppSucc;                     /* 发送成功的消息总长 */

    unsigned int frameCount;                          /* 总的发送帧数 */
    unsigned int frameCountOfSelf;                    /* 发送的自身帧数 */
    unsigned int frameCountOfPass;                    /* 发送的转发帧数 */
    unsigned int frameCountOfAck;                     /* 发送的ACK帧数 */
    unsigned int frameCountOfFailAsCheck;             /* 对方回复校验失败的发送帧数 */
    unsigned int frameCountOfFailAsAckTimeout;        /* 对方回复超时的发送帧数 */
    unsigned int frameCountOfDiscardRecvAck;          /* 丢弃的收到ACK数 */
...
{
public:
    SERIAL_SENDER_C(SERIAL_C &objSerial, SERIAL_ADDR_ENUM enLocAddr, SERIAL_ADDR_ENUM enPeerAddr, unsigned int maxListSize = SERIAL_SNEDER_MSG_BUFF_SIZE)
    ...
    int Send(SERIAL_ADDR_ENUM enTgtAddr, unsigned char msgType, const unsigned char *pucMsg, unsigned short msgLen);
    void  SendAck(const SERIAL_FRAME_C &objRecvFrame, bool isCheckSucc);
    int SendPass(const unsigned char *pucData, unsigned short dataLen);
...
private:
    ...
    const unsigned int                        maxListSize_;

    std::mutex                          objMutex_;
    SERIAL_SENDER_DBG_INFO_STRU         stDbgInfo_;
    unsigned char                              aucAddrFrameNo_[SERIAL_ADDR_BUTT];
    ...
    unsigned int                              sendingFrameSendCount_       = 0;            /* 为0表示当前没有正在发送帧 */
    ...
    unsigned char AllocFrameNo(SERIAL_ADDR_ENUM enTgtAddr);
    bool   IsConnectTimeout(void);

    void   SendFrameImmediately(const SERIAL_FRAME_C &objFrame);
    int  SendFrameSequentially(const SERIAL_FRAME_C &objFrame);
