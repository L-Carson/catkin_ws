...
#include <stdio.h>          /* 标准输入输出       getchar() */
#include <thread>           /* 标准线程库 std::this_thread::sleep_for */
#include <algorithm>        /* 标准算法库 std::find_if */
...
int WaitAndGetInputVal(const INPUT_INFO_STRU astInputInfo[], unsigned int numOfInputInfo)
{
    ...
    for (unsigned int loop = 0; loop < numOfInputInfo; ++loop) {
