#include "comm_socketcan_sender.h"

void SOCKETCAN_SENDER_C::PeriodUpt(double curTime)
{
...
INT32 SOCKETCAN_SENDER_C::Send(SERIAL_ADDR_ENUM enTgtAddr, UINT08 msgType, const UINT08 *pucMsg, UINT16 msgLen)
{
    std::unique_lock<std::mutex> objLocker(objMutex_);
...
void SOCKETCAN_SENDER_C::SendAck(const SERIAL_FRAME_C &objRecvFrame, BOOL isCheckSucc)
{
    std::unique_lock<std::mutex> objLocker(objMutex_);
...
INT32 SOCKETCAN_SENDER_C::SendPass(const UINT08 *pucData, UINT16 dataLen)
{
    std::unique_lock<std::mutex> objLocker(objMutex_);
...
void SOCKETCAN_SENDER_C::ProcAck(const SERIAL_FRAME_C &objAckFrame)
{
    std::unique_lock<std::mutex> objLocker(objMutex_);
...
BOOL SOCKETCAN_SENDER_C::IsConnectFault(void)
{
    std::unique_lock<std::mutex> objLocker(objMutex_);
...
void SOCKETCAN_SENDER_C::OutputDbgInfo(void)
{
    std::unique_lock<std::mutex> objLocker(objMutex_);
...
UINT08 SOCKETCAN_SENDER_C::AllocFrameNo(SERIAL_ADDR_ENUM enTgtAddr)
{
...
BOOL SOCKETCAN_SENDER_C::IsConnectTimeout(void)
{
    return ((curTime_ - lastSendFrameSuccTime_) > SERIAL_SENDER_FAULT_NO_SUCC_TIMEOUT);
}

void SOCKETCAN_SENDER_C::SendFrameImmediately(const SERIAL_FRAME_C &objFrame)
{
    SERIAL_FRAME_BUFF_P_STRU stBuff;
    objFrame.GetBuff(stBuff);

    objSocketCan_.SendData(stBuff.pucBuff, stBuff.buffLen);
...
修改记录:
**************************************************************************************/
INT32 SOCKETCAN_SENDER_C::SendFrameSequentially(const SERIAL_FRAME_C &objFrame)
{
    ++stDbgInfo_.msgCount;
...
修改记录:
**************************************************************************************/
void SOCKETCAN_SENDER_C::SendHandshakeMsg(void)
{
...
void SOCKETCAN_SENDER_C::ReTrySendingFrame(void)
{
    SERIAL_FRAME_BUFF_P_STRU stBuff;
...
void SOCKETCAN_SENDER_C::EndSendingFrame(void)
{
...
void SOCKETCAN_SENDER_C::ProcSendingFrameAckFail(void)
{
...
void SOCKETCAN_SENDER_C::ProcSendingFrameAck(const SERIAL_FRAME_C &objAckFrame)
{
...
void SOCKETCAN_SENDER_C::ProcSendingFrameAckTimeout(void)
{
...
void SOCKETCAN_SENDER_C::UptSendingFrameState(void)
{
    if (   (sendingFrameSendCount_ == 0)
...
修改记录:
**************************************************************************************/
void SOCKETCAN_SENDER_C::TrySendNextFrame(void)
{
    ...
    objSocketCan_.SendData(stFirstFrameBuff.aucBuff, stFirstFrameBuff.buffLen);
...
修改记录:
**************************************************************************************/
void SOCKETCAN_SENDER_C::TryHandshake(void)
{
    /* 一段时间没有发送消息，则尝试握手 */
