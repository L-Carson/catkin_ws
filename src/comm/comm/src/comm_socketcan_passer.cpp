...
INT32 SOCKETCAN_PASSER_C::Init(void)
{
    ST_LOG_INFO("Succ.");
    return 0;
}

void SOCKETCAN_PASSER_C::RegPassIn(SERIAL_ADDR_ENUM enTgtAddr, const std::string &strDataTopic)
{
    SOCKETCAN_PASS_IN_SENDER_INFO_STRU stSenderInfo;
...
void SOCKETCAN_PASSER_C::RegPassOut(const std::string &strDataTopic)
{
    SERIAL_PASS_DATA_RECEIVER_C *pobjReceiver = new SERIAL_PASS_DATA_RECEIVER_C(nh_, strDataTopic, std::bind(&SOCKETCAN_PASSER_C::RecvPassOutData, this, std::placeholders::_1, std::placeholders::_2));
...
INT32 SOCKETCAN_PASSER_C::Pass(SERIAL_ADDR_ENUM enTgtAddr, const UINT08 *pucMsg, UINT32 msgLen)
{
...
void SOCKETCAN_PASSER_C::ClearPassInDataSenders(void)
{
    for (SOCKETCAN_PASS_IN_SENDER_INFO_STRU &stSenderInfo : vectPassInSenderInfo_) {
...
void SOCKETCAN_PASSER_C::ClearPassOutDataReceivers(void)
{
...
void SOCKETCAN_PASSER_C::RecvPassOutData(const UINT08 *pucMsg, UINT32 msgLen)
{
    INT32 rslt = objSocketcanSender_.SendPass(pucMsg, UINT16(msgLen));
    ST_LOG_ERR_SAMPLE_IF_FAIL(rslt, 100, "objSocketcanSender_.SendPass() Fail.");
}

SERIAL_PASS_DATA_SENDER_C *SOCKETCAN_PASSER_C::GetPassInDataSender(SERIAL_ADDR_ENUM enTgtAddr)
{
    for (SOCKETCAN_PASS_IN_SENDER_INFO_STRU &stSenderInfo : vectPassInSenderInfo_) {
