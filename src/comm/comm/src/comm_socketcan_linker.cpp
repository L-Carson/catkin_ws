...
#include "comm/comm_socketcan_linker.h"

/**************************************************************************************
功能描述: 串口管理 初始化
修改记录:
**************************************************************************************/
INT32 SOCKETCAN_LINKER_C::Init(const std::string &strDevName, const SOCKETCAN_LINKER_RECV_MSG_FUNC_C &objRecvMsgFunc)
{
    ST_LOG_INFO("LocAddr=%u, PeerAddr=%u, DevName=%s.", enLocAddr_, enPeerAddr_, strDevName.c_str());

    INT32 rslt = objSocketcan_.Open(strDevName.c_str(), SOCKETCAN_LINKER_BANDRATE);
    if (rslt != 0) {
        ST_LOG_ERR("objSocketcan_.Open(%s) Fail.", strDevName.c_str());
        return -1;
    }
...
修改记录:
**************************************************************************************/
void SOCKETCAN_LINKER_C::RegRecvPassMsg(SERIAL_ADDR_ENUM enTgtAddr, const SOCKETCAN_LINKER_RECV_PASS_MSG_FUNC_C &objRecvMsgFunc)
{
    ST_LOG_INFO("Reg Recv Pass Msg Addr %u.", enTgtAddr);

    SOCKETCAN_LINKER_RECV_PASS_MSG_REG_STRU stReg;
...
修改记录:
**************************************************************************************/
void SOCKETCAN_LINKER_C::RegPassInMsg(SERIAL_ADDR_ENUM enTgtAddr, const std::string &strDataTopic)
{
...
修改记录:
**************************************************************************************/
void SOCKETCAN_LINKER_C::RegPassOutMsg(const std::string &strDataTopic)
{
...
修改记录:
**************************************************************************************/
INT32 SOCKETCAN_LINKER_C::Send(SERIAL_ADDR_ENUM enTgtAddr, UINT08 msgType, const UINT08 *pucMsg, UINT32 msgLen)
{
    return objSender_.Send(enTgtAddr, msgType, pucMsg, UINT16(msgLen));
}

INT32 SOCKETCAN_LINKER_C::SendPassMsg(const UINT08 *pucData, UINT32 dataLen)
{
...
修改记录:
**************************************************************************************/
BOOL SOCKETCAN_LINKER_C::IsFault(void)
{
...
修改记录:
**************************************************************************************/
void SOCKETCAN_LINKER_C::StatisReadCycleLen(void)
{
...
修改记录:
**************************************************************************************/
void SOCKETCAN_LINKER_C::OutputDbgInfo(double curTime)
{
...
const SOCKETCAN_LINKER_RECV_PASS_MSG_REG_STRU *SOCKETCAN_LINKER_C::GetAppPassMsgReg(SERIAL_ADDR_ENUM enTgtAddr)
{
    for (const SOCKETCAN_LINKER_RECV_PASS_MSG_REG_STRU &stRecvPassMsgReg : listRecvPassMsgReg_) {
...
BOOL SOCKETCAN_LINKER_C::IsAppPassFrame(const SERIAL_FRAME_C &objFrame)
{
...
修改记录:
**************************************************************************************/
BOOL SOCKETCAN_LINKER_C::IsFrameRepeat(const SERIAL_FRAME_C &objFrame)
{
...
void SOCKETCAN_LINKER_C::ProcSelfAppFrame(const SERIAL_FRAME_C &objFrame)
{
...
修改记录:
**************************************************************************************/
void SOCKETCAN_LINKER_C::ProcSelfFrame(const SERIAL_FRAME_C &objFrame)
{
...
void SOCKETCAN_LINKER_C::ProcAppPassFrame(const SERIAL_FRAME_C &objFrame)
{
    ++stDbgInfo_.frameCountOfPassByApp;

    const SOCKETCAN_LINKER_RECV_PASS_MSG_REG_STRU *pstPassMsgReg = GetAppPassMsgReg(objFrame.GetTgtAddr());
...
修改记录:
**************************************************************************************/
void SOCKETCAN_LINKER_C::ProcPassFrame(const SERIAL_FRAME_C &objFrame)
{
...
void SOCKETCAN_LINKER_C::ProcFrame(const SERIAL_FRAME_C &objFrame, BOOL isCheckSucc)
{
    /* 本地处理的非ACK帧需要回复ACK */
...
BOOL SOCKETCAN_LINKER_C::ReadData(SOCKETCAN_LINKER_RECV_DATA_STRU &stRecvDataInfo)
{
    ...
    INT32  readDataLen = objSocketcan_.ReadData(&stRecvDataInfo.aucData[stRecvDataInfo.dataLen], freeBuffLen);
    if ((readDataLen < 0) || (readDataLen > INT32(freeBuffLen))) {
        ...
        if ((stRecvDataInfo.dataLen > 0) && ((Comm_GetTimeSec() - stRecvDataInfo.lastRecvDataTime) > SOCKETCAN_LINKER_SEND_INTERVAL_MAX)) {
...
修改记录:
**************************************************************************************/
UINT16 SOCKETCAN_LINKER_C::ProcData(const UINT08 *pucData, UINT16 dataLen)
{
...
void SOCKETCAN_LINKER_C::PushFrame(const SERIAL_FRAME_C &objFrame, BOOL isCheckSucc)
{
    std::unique_lock<std::mutex> objLocker(objRecvFrameListMutex_);

    UINT16 frameLen         = objFrame.GetFrameLen();
    UINT16 recvFrameBuffLen = sizeof(SOCKETCAN_LINKER_RECV_FRAME_STRU) + frameLen;
    SOCKETCAN_LINKER_RECV_FRAME_STRU *pstRecvFrame = (SOCKETCAN_LINKER_RECV_FRAME_STRU *)new UINT08[recvFrameBuffLen];
...
const SOCKETCAN_LINKER_RECV_FRAME_STRU *SOCKETCAN_LINKER_C::PopFrame(void)
{
    std::unique_lock<std::mutex> objLocker(objRecvFrameListMutex_);
    ...
    const SOCKETCAN_LINKER_RECV_FRAME_STRU *pstFirst = listRecvFrame_.front();
...
修改记录:
**************************************************************************************/
void SOCKETCAN_LINKER_C::ReadDataTask(void)
{
    ST_LOG_INFO("Begin.");

    SOCKETCAN_LINKER_RECV_DATA_STRU stRecvDataInfo;
...
void SOCKETCAN_LINKER_C::ProcFrameTask(void)
{
    ST_LOG_INFO("Begin.");
    ...
    while (isThreadWork_)
    {
        ...
        const SOCKETCAN_LINKER_RECV_FRAME_STRU *pstRecvFrame = PopFrame();
...
修改记录:
**************************************************************************************/
INT32 SOCKETCAN_LINKER_C::CreateThread(void)
{
    ...
    pobjReadDataThread_ = new std::thread(&SOCKETCAN_LINKER_C::ReadDataTask, this);
    if (pobjReadDataThread_ == NULL) {
        ST_LOG_ERR("New ReadDataTask Fail.");
        return -1;
    }

    pobjProcFrameThread_ = new std::thread(&SOCKETCAN_LINKER_C::ProcFrameTask, this);
...
修改记录:
**************************************************************************************/
void SOCKETCAN_LINKER_C::DeleteThread(void)
{
    ST_LOG_INFO("Begin.");
