...
{
    UINT16 dataLen;
    UINT08 aucData[SERIAL_MSG_MAX_LEN * 2];     /* 为了能合并连续两次接收的消息，所以该最大长度翻倍 */;
...
using SERIAL_LINKER_RECV_MSG_FUNC_C      = std::function<void(SERIAL_ADDR_ENUM, UINT08, const UINT08 *, UINT32)>;
using SERIAL_LINKER_RECV_PASS_MSG_FUNC_C = std::function<void(const UINT08 *, UINT32)>;

/**************************************************************************************
功能描述: 消息代收，主要是给本地测试时使用
修改记录:
**************************************************************************************/
struct SERIAL_LINKER_RECV_PASS_MSG_REG_STRU
{
    SERIAL_ADDR_ENUM                   enTgtAddr;
    SERIAL_LINKER_RECV_PASS_MSG_FUNC_C objRecvMsgFunc;
};
...
public:
    ...
    INT32 Init(const std::string &strDevName, const SERIAL_LINKER_RECV_MSG_FUNC_C &objRecvMsgFunc);
    void  RegRecvPassMsg(SERIAL_ADDR_ENUM enTgtAddr, const SERIAL_LINKER_RECV_PASS_MSG_FUNC_C &objRecvMsgFunc);
...
private:
    ...
    SERIAL_PASSER_C                             objPasser_;
    SERIAL_LINKER_RECV_MSG_FUNC_C               objProcMsgFunc_;
    list<SERIAL_LINKER_RECV_PASS_MSG_REG_STRU>  listRecvPassMsgReg_;
    ...
    const SERIAL_LINKER_RECV_PASS_MSG_REG_STRU *GetAppPassMsgReg(SERIAL_ADDR_ENUM enTgtAddr);
