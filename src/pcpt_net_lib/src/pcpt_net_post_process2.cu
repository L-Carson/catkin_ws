...
/**************************************************************************************
功能描述: box_decode_warp2
修改记录:
**************************************************************************************/
__device__ void box_decode_warp2(int head_offset, const float* box_pred, int tid , int num_per_head ,
                                int counter, float* filtered_box)
{
    int x_inds  =  tid % 78 ;
    int y_inds  =  tid / 78 ;
    filtered_box[blockIdx.z * num_per_head * 7  + counter * 7 + 0] =(box_pred[ head_offset + num_per_head * 0 + tid ]+x_inds)*4*0.2-31.2;
    filtered_box[blockIdx.z * num_per_head * 7  + counter * 7 + 1] =(box_pred[ head_offset + num_per_head * 1 + tid ]+y_inds)*4*0.2-31.2;
    filtered_box[blockIdx.z * num_per_head * 7  + counter * 7 + 2] = box_pred[ head_offset + num_per_head * 2 + tid ];
    filtered_box[blockIdx.z * num_per_head * 7  + counter * 7 + 3] = box_pred[ head_offset + num_per_head * 3 + tid ];
    filtered_box[blockIdx.z * num_per_head * 7  + counter * 7 + 4] = box_pred[ head_offset + num_per_head * 4 + tid ];
    filtered_box[blockIdx.z * num_per_head * 7  + counter * 7 + 5] = box_pred[ head_offset + num_per_head * 5 + tid ];
    filtered_box[blockIdx.z * num_per_head * 7  + counter * 7 + 6] = box_pred[ head_offset + num_per_head * 6 + tid ];
}


/**************************************************************************************
功能描述: _filter_kernel2 cls_pre sigmod
修改记录:
**************************************************************************************/
__global__ void  _filter_kernel2(const float* cls_pred_0,const float* cls_pred_1,const float* cls_pred_2,
                            const float* box_pred_0,const float* box_pred_1,const float* box_pred_2,
                            float* filtered_box,float* filtered_score,int* filter_count,const float score_threshold)
{
    int num_per_head = gridDim.x * gridDim.y * blockDim.x;
    extern __shared__ float cls_score[];
    int tid = blockIdx.x * gridDim.y * blockDim.x + blockIdx.y *  blockDim.x + threadIdx.x;

    if ( blockIdx.z == 0) {
        cls_score[ threadIdx.x ] =  cls_pred_0[ tid ];
    }
    if ( blockIdx.z == 1) {
        cls_score[ threadIdx.x ] = cls_pred_1[tid];
    }
    if ( blockIdx.z == 2) {
        cls_score[ threadIdx.x ] = cls_pred_2[tid];
    }
    ...
    if( cls_score[ threadIdx.x ] > score_threshold)
    {
        ...
        if ( blockIdx.z == 0) {
            box_decode_warp2(0 ,box_pred_0 , tid , num_per_head , counter , filtered_box);
            filtered_score[blockIdx.z * num_per_head + counter] = cls_score[ threadIdx.x ];
        }else
        if ( blockIdx.z == 1) {
            box_decode_warp2(0 ,box_pred_1 , tid , num_per_head , counter , filtered_box);
            filtered_score[blockIdx.z * num_per_head + counter] = cls_score[ threadIdx.x ];
        }else
        if ( blockIdx.z == 2) {
            box_decode_warp2(0 ,box_pred_2 , tid , num_per_head , counter , filtered_box);
            filtered_score[blockIdx.z * num_per_head + counter] = cls_score[ threadIdx.x ];
        }
    }
...
/**************************************************************************************
功能描述: sort_boxes_by_indexes_kernel2
修改记录:
**************************************************************************************/
__global__ void sort_boxes_by_indexes_kernel2(float* filtered_box, float* filtered_scores, int* indexes, int filter_count,
...
/**************************************************************************************
功能描述: PostprocessCuda2
修改记录:
**************************************************************************************/
PostprocessCuda2::PostprocessCuda2(const int num_threads, const float float_min, const float float_max,
...
修改记录:
**************************************************************************************/
void PostprocessCuda2::DoPostprocessCuda(const float* box_preds, float* dev_filtered_box, float* dev_filtered_score,
    int* dev_filter_count, std::vector<float>& out_detection, std::vector<int>& out_label, std::vector<float>& out_score)
{
    dim3 gridsize(9, 4 , 3);
    _filter_kernel2<<< gridsize, 169 , 169 * 2 * sizeof(float)>>>(
                    &box_preds[0],
                    &box_preds[1 * 78*78 * 8],
                    &box_preds[2 * 78*78 * 8],
                    &box_preds[78 *78 ],
                    &box_preds[1 * 78 * 78 * 8 + 78*78],
                    &box_preds[2 * 78 * 78 * 8 + 78*78],
                    dev_filtered_box,
                    dev_filtered_score,
                    dev_filter_count,
                    score_threshold_);
    ...
    for (int i = 0; i < num_class_; ++i)
    {
        ...
        sort_boxes_by_indexes_kernel2<<<num_blocks, num_threads_>>>(
        ...
        cudaMemcpy(host_filtered_box, dev_sorted_filtered_box, host_filter_count[i] * num_output_box_feature_ *
