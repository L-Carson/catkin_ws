...
{
public:
    /*num_threads 启动cuda内核时的线程数 num_anchor_per_cls每个类别的锚点数量*/
    PostprocessCuda(const int num_threads, const float float_min, const float float_max,
    ...
    void DoPostprocessCuda(float* cls_pred_0, const float* box_preds, float* dev_filtered_box,
