...
修改记录:
**************************************************************************************/
class PointPillars : public Net_Base
{
public:
    /*score_threshold 过滤输出的分数阈值 nms_overlap_threshold NMS的IOU阈值*/
    PointPillars(const float scoreThreshold, const float nmsOverlapThreshold,
                 const bool useOnnx, const std::string modelFile, float *anchorsData, int classNum);

    ~PointPillars() {
        GPU_CHECK(cudaFree(devAnchors_));
        ...
        GPU_CHECK(cudaFree(rpnBuffers_[1]));
 ...
 private:
    ...
    int                 kNumAnchorPerCls_;
    int                 kNumAnchorXinds_;
    int                 kNumAnchorYinds_;
    int                 kNumAnchor_;
    ...
    std::unique_ptr<PostprocessCuda>        postprocessCudaPtr_;
