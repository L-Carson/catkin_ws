...
class visionRects {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.rects[i] = visionRect.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.rects.forEach((val) => {
      length += visionRect.getMessageSize(val);
    });
    return length + 9;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new visionRects(null);
    ...
    if (msg.rects !== undefined) {
      ...
      for (let i = 0; i < resolved.rects.length; ++i) {
        resolved.rects[i] = visionRect.Resolve(msg.rects[i]);
      }
    }
    else {
      resolved.rects = []
