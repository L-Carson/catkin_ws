...
class coor2DArrayLA {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      this.hasRisk = null;
      this.coor2DArrayLA = null;
    }
    else {
      if (initObj.hasOwnProperty('hasRisk')) {
        this.hasRisk = initObj.hasRisk
      }
      else {
        this.hasRisk = false;
      }
      if (initObj.hasOwnProperty('coor2DArrayLA')) {
        this.coor2DArrayLA = initObj.coor2DArrayLA
      }
      else {
        this.coor2DArrayLA = [];
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Serialize message field [hasRisk]
    bufferOffset = _serializer.bool(obj.hasRisk, buffer, bufferOffset);
    // Serialize message field [coor2DArrayLA]
    // Serialize the length for message field [coor2DArrayLA]
    bufferOffset = _serializer.uint32(obj.coor2DArrayLA.length, buffer, bufferOffset);
    obj.coor2DArrayLA.forEach((val) => {
      bufferOffset = coor2DArrayL.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new coor2DArrayLA(null);
    // Deserialize message field [hasRisk]
    data.hasRisk = _deserializer.bool(buffer, bufferOffset);
    // Deserialize message field [coor2DArrayLA]
    // Deserialize array length for message field [coor2DArrayLA]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.coor2DArrayLA = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.coor2DArrayLA[i] = coor2DArrayL.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    let length = 0;
    object.coor2DArrayLA.forEach((val) => {
      length += coor2DArrayL.getMessageSize(val);
    });
    return length + 5;
  }
  ...
  static messageDefinition() {
    ...
    return `
    bool              hasRisk       #图像视野有无风险，如有风险，需要规控减速
  ...
  static Resolve(msg) {
    ...
    const resolved = new coor2DArrayLA(null);
    if (msg.hasRisk !== undefined) {
      resolved.hasRisk = msg.hasRisk;
    }
    else {
      resolved.hasRisk = false
    }

    if (msg.coor2DArrayLA !== undefined) {
      resolved.coor2DArrayLA = new Array(msg.coor2DArrayLA.length);
      for (let i = 0; i < resolved.coor2DArrayLA.length; ++i) {
        resolved.coor2DArrayLA[i] = coor2DArrayL.Resolve(msg.coor2DArrayLA[i]);
      }
    }
    else {
      resolved.coor2DArrayLA = []
    }
