...
class movTarget {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      if (initObj.hasOwnProperty('movPredArray')) {
        this.movPredArray = initObj.movPredArray
      }
      else {
        this.movPredArray = [];
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Serialize message field [movPredArray]
    // Serialize the length for message field [movPredArray]
    bufferOffset = _serializer.uint32(obj.movPredArray.length, buffer, bufferOffset);
    obj.movPredArray.forEach((val) => {
      bufferOffset = movPred.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new movTarget(null);
    // Deserialize message field [id]
    data.id = _deserializer.uint32(buffer, bufferOffset);
    // Deserialize message field [movPredArray]
    // Deserialize array length for message field [movPredArray]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.movPredArray = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.movPredArray[i] = movPred.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new movTarget(null);
    if (msg.id !== undefined) {
      resolved.id = msg.id;
    }
    else {
    ...
    if (msg.movPredArray !== undefined) {
      resolved.movPredArray = new Array(msg.movPredArray.length);
      for (let i = 0; i < resolved.movPredArray.length; ++i) {
        resolved.movPredArray[i] = movPred.Resolve(msg.movPredArray[i]);
      }
    }
    else {
      resolved.movPredArray = []
    }
